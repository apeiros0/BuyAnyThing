...
export default {
  ...
  methods: {
    getCoupons(page = 1) {
      this.page = page;
      const api = `${process.env.API_URL}/api/${process.env.API_PATH}/admin/coupons?page=${page}`;
      const self = this;
      self.isLoading = true;
    ...
    updateCoupon() {
      // 新增優惠券
      let api = `${process.env.API_URL}/api/${process.env.API_PATH}/admin/coupon`;
      let APIMethod = 'post';
      const self = this;
      // 把 到期日 轉換成 timeStamp
      ...
      // 編輯優惠券
      if (!self.isNew) {
        api = `${process.env.API_URL}/api/${process.env.API_PATH}/admin/coupon/${self.tempCoupon.id}`;
        APIMethod = 'put';
        newObject = {
    ...
    deleteCoupon() {
      const self = this;
      const api = `${process.env.API_URL}/api/${process.env.API_PATH}/admin/coupon/${self.tempCoupon.id}`;
